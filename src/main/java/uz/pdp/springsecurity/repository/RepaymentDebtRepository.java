...
public interface RepaymentDebtRepository extends JpaRepository<RepaymentDebt, UUID> {
    @Query("select sum(r.debtSum) from RepaymentDebt r where r.customer.branch.id= :branchId and r.paymentMethod.type = :paymentMethodName and r.createdAt >= :startDate AND r.createdAt <= :endDate and r.delete = false ")
    ...
    @Query("select sum(r.debtSum) from RepaymentDebt r where r.customer.branch.business.id= :businessId and  r.paymentMethod.type = :paymentMethodName and r.createdAt >= :startDate AND r.createdAt <= :endDate and r.delete = false ")
